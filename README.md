...
> Answer: The Page is only initially loaded once, and then the user can navigate through the page without load times.
...
> Answer: They are components that are controlled by react using state. As apposed to something that is controlled by the browser DOM
